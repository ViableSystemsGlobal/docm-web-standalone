...
"[project]/.next-internal/server/app/api/homepage/route/actions.js [app-rsc] (server actions loader, ecmascript)": (function(__turbopack_context__) {
...
"[project]/src/app/api/homepage/route.ts [app-route] (ecmascript)": ((__turbopack_context__) => {
"use strict";
...
async function GET(request) {
    try {
        ...
        const { data: homePage, error: pageError } = await supabase.from('pages').select('*').in('slug', [
            'home',
            'homepage',
            'index'
        ...
        const { data: sections, error: sectionsError } = await supabase.from('page_sections').select('*').eq('page_id', homePage.id).order('order', {
            ascending: true
        });
        ...
        let getInvolvedTemplates = [];
        const hasGetInvolvedSection = sections.some((section)=>section.type === 'get_involved');
        if (hasGetInvolvedSection) {
            try {
                const { data: templates, error: getInvolvedError } = await supabase.from('get_involved_templates').select(`
            id,
            title,
            excerpt,
            description,
          ...
          `).eq('status', 'published').order('priority_order', {
                    ascending: true
                }).limit(6);
                if (!getInvolvedError && templates && templates.length > 0) {
                    // Get all groups to match by name when ministry_group_id is null
                    const { data: allGroups } = await supabase.from('groups').select('id, name, type, image_url').eq('status', 'active');
                    console.log('🔍 Available groups for matching:', allGroups?.map((g)=>({
                            name: g.name,
                    ...
                    const templatesWithImages = templates.map((template)=>{
                        ...
                        if (template.ministry_group?.image_url) {
                            groupImage = template.ministry_group.image_url;
                        } else if (allGroups) {
                            const matchingGroup = allGroups.find((group)=>group.name.toLowerCase() === template.title.toLowerCase() || group.name.toLowerCase().includes(template.title.toLowerCase()) || template.title.toLowerCase().includes(group.name.toLowerCase()));
                            if (matchingGroup) {
                                console.log(`🎯 Matched template "${template.title}" with group "${matchingGroup.name}" (image: ${!!matchingGroup.image_url})`);
                                groupImage = matchingGroup.image_url;
                            } else {
            ...
            `).eq('status', 'active').order('created_at', {
                    ...
                    if (!groupsError && groups && groups.length > 0) {
                        ...
                        getInvolvedTemplates = groups.map((group, index)=>({
                                ...
                                featured_image: group.image_url || null,
...
function buildHomepageContent(sections, getInvolvedTemplates) {
    const content = {};
    sections.forEach((section)=>{
        ...
        switch(section.type){
            case 'hero':
                ...
                const isVideo = backgroundMedia && (backgroundMedia.includes('.mp4') || backgroundMedia.includes('.webm') || backgroundMedia.includes('.mov') || backgroundMedia.includes('.avi'));
                content.hero = {
            ...
            case 'testimonial_slider':
                ...
                const mappedTestimonials = adminTestimonials.map((testimonial)=>({
...
function getGroupIcon(name, type) {
...
function getGroupGradient(index) {
    const gradients = [
        {
            from: 'blue-800',
            to: 'indigo-900'
...
//# sourceMappingURL=%5Broot-of-the-server%5D__8eedb3ce._.js.map